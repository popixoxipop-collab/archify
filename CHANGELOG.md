...
- **Geometric overlap checkers for hand-placed SVG.** `scripts/check-line-overlaps.mjs` (`npm run check:overlaps`) reports connector segments that pass through the real geometry of a sibling shape they aren't part of — true polygon containment for diamonds/custom shapes, not just bounding box, so it doesn't false-positive near pointed corners. `scripts/check-text-overlaps.mjs` (`npm run check:text-overlaps`, optional Playwright dependency, not wired into `npm test`) reports text-vs-text/text-vs-box/text-vs-line overlaps measured via a real browser's `getBBox()`/`getCTM()` rather than a static width estimate. Both close a gap `check-render-output.mjs` leaves by design: its arrow-crossing check is scoped to the legend and to archify's own `a-*` class convention, so it can't catch a hand-authored connector or label overlapping an arbitrary sibling node — the exact failure mode hand-placed SVG (custom shapes especially) can produce but renderer-only output can't. Found 16 line-overlap and 29 text-overlap real defects on a hand-placed multi-lane diagram with custom gate shapes across two rounds; see SKILL.md's "Geometric overlap checkers" section.
- **Documented hybrid custom-shape injection.** New SKILL.md section "Hybrid custom-shape injection" documents rendering normally with placeholder component types, then swapping specific nodes' two-rect pattern for hand-authored path geometry (keeping the original `c-mask`/`c-<type>` classes so the theme toggle still works) when a diagram needs a shape outside the typed renderers' component-type enum — e.g. logic gates — without giving up lane/legend/theme machinery entirely. Also documents a readability lesson from the same case study: passing both overlap checkers with 0 findings rules out geometric overlap, not label-to-shape ambiguity in dense clusters — verify those by eye, by tracing connector lines, not by checker output alone.
